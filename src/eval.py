...
def evaluate(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    model = model.load_from_checkpoint(cfg.ckpt_path, strict=False, train_loss=None, val_loss=None)

    # import torch
    # data = "fiqa"
    # torch.save(model.model.bert, f"/workspace/tmp/{data}-bert.pt")
    # torch.save(model.model.linear, f"/workspace/tmp/{data}-linear.pt")
    #
    #
    ...
    trainer.test(
        model=model,
        datamodule=datamodule,
        ckpt_path=cfg.ckpt_path if cfg.ckpt_path.split(".")[-1] == "pt" else None,
    )
