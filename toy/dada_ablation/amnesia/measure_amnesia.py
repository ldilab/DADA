...
def load_mlm(experiment_name, data_name):
    print("Loading MLM...")
    mlm = {}
    with (amnesia_dir / experiment_name / data_name / f"mlm.jsonl").open() as f:
        lines = f.readlines()
        for line in tqdm(lines, desc="Loading MLM"):
...
def main_run(k: int = 100):
    ...
    for dataset in datasets:
        for experiment in experiments:
            ...
            mlms = load_mlm(experiment, dataset)
            for did, mlm in tqdm(mlms.items(), desc="Measuring Amnesia"):
                idf_toks, mlm_toks = extract_topk(idf, mlm, k=k)
                intersection = measure_intersection(idf_toks, mlm_toks)
                jaccard = measure_jaccard_distance(idf_toks, mlm_toks)
                missed = measure_missed(idf_toks, mlm_toks)

                # intersections[did] = intersection
                jaccards[did] = jaccard
                # misseds[did] = missed
